		...
		while (1)
		{
			...
			else if (gInventory.isCategoryComplete(info.mCatUUID))
			{
				...
				if (mTimelyFetchPending && mFetchTimer.getElapsedTimeF32() < fast_fetch_time)
				{
					...
					LL_DEBUGS("InventoryFetch") << "Inventory fetch times shrunk to ("
												<< mMinTimeBetweenFetches << ", "
												<< mMaxTimeBetweenFetches << ")"
												<< LL_ENDL;
				}
...
void LLInventoryModelFetchDescendentsResponder::result(const LLSD& content)
{
			...
			folder_it != content["folders"].endArray(); ++folder_it)
		{
            ...
            if (parent_id.isNull())
            {
			    ...
			    for (LLSD::array_const_iterator item_it = folder_sd["items"].beginArray();
				    item_it != folder_sd["items"].endArray(); ++item_it)
			    {
	...
	if (content.has("bad_folders"))
	{
		for (LLSD::array_const_iterator folder_it = content["bad_folders"].beginArray();
			folder_it != content["bad_folders"].endArray(); ++folder_it)
		{
			LLSD folder_sd = *folder_it;
...
void LLInventoryModelFetchDescendentsResponder::error(U32 status, const std::string& reason)
{
	...
	if (status == 499) // timed out
	{
		for (LLSD::array_const_iterator folder_it = mRequestSD["folders"].beginArray();
			folder_it != mRequestSD["folders"].endArray(); ++folder_it)
		{
			LLSD folder_sd = *folder_it;
...
{
	// Background fetch is called from gIdleCallbacks in a loop until background
	// fetch is stopped.
	// If there are items in mFetchQueue, we want to check the time since the
	// last bulkFetch was sent.
	// If it exceeds our retry time, go ahead and fire off another batch.  
	...
	if (gDisconnected || mBulkFetchCount > max_concurrent_fetches ||
