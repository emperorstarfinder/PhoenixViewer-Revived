...
void LLVivoxProtocolParser::EndTag(const char *tag)
{
	...
	if (!ignoringTags)
	{
		...
		{
			LLVoiceClient::getInstance()->addCaptureDevice(deviceString);
		}
		else if (!stricmp("RenderDevice", tag))
		{
			LLVoiceClient::getInstance()->addRenderDevice(deviceString);
		}
...
void LLVivoxProtocolParser::processResponse(std::string tag)
{
	...
	if (isEvent)
	{
		...
		else if (!stricmp(eventTypeCstr, "MediaCompletionEvent"))
		{
			...
			*/
			LLVoiceClient::getInstance()->mediaCompletionEvent(sessionGroupHandle, mediaCompletionType);
		}
