...
public:
	...
	static void		copyInto(std::basic_string<T>& dst, const std::basic_string<T>& src, size_type offset);	static bool		isPartOfWord(T c) { return (c == (T)'_') || LLStringOps::isAlnum(c); }
 ...
 */
LL_COMMON_API std::string ll_convert_wide_to_string(const wchar_t* in, unsigned int code_page = 0);
